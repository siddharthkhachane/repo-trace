...
function exportChatHistory() {
  ...
  const repoInfo = currentRepoId || 'No repository';
  const repoStats = currentRepoStats ? 
    `\n**Repository Stats:**\n- Total Files: ${Number(currentRepoStats.total_files || 0).toLocaleString()}\n- Lines of Code: ${Number(currentRepoStats.total_loc || 0).toLocaleString()}\n` : '';

  let markdown = `# Repo-Trace Chat Export\n\n`;
  markdown += `**Exported:** ${new Date().toLocaleString()}\n\n`;
  markdown += `**Repository:** ${repoInfo}\n`;
  markdown += repoStats;
  markdown += `\n---\n\n`;
  ...
  messages.forEach((message, index) => {
    ...
    if (codeBlocks.length > 0) {
      // Process node by node to preserve code blocks
      let tempDiv = bubble.cloneNode(true);
      tempDiv.querySelectorAll('.code-block').forEach((block, i) => {
        const lang = block.querySelector('.code-header span')?.textContent?.toLowerCase() || 'plaintext';
        const code = block.querySelector('code')?.textContent || '';
        const placeholder = `__CODE_BLOCK_${i}__`;
        const codeMarkdown = `\n\`\`\`${lang}\n${code}\n\`\`\`\n`;
        block.replaceWith(document.createTextNode(placeholder));
        tempDiv.innerHTML = tempDiv.innerHTML.replace(placeholder, codeMarkdown);
      });
    ...
    if (citations) {
      markdown += `**Sources:**\n\n`;
      const citationLinks = citations.querySelectorAll('.citation-link');
      citationLinks.forEach(link => {
        markdown += `- ${link.textContent.trim()}\n`;
      });
      markdown += `\n`;
    }
    
    // Add referenced files if present
    const referencedFiles = message.querySelector('.referenced-files-sidebar');
    if (referencedFiles) {
      markdown += `**Referenced Files:**\n\n`;
      const fileItems = referencedFiles.querySelectorAll('.file-item');
      fileItems.forEach(item => {
        const fileName = item.querySelector('.file-name')?.textContent || '';
        const filePath = item.querySelector('.file-path')?.textContent || '';
        const relevance = item.querySelector('.file-relevance')?.textContent || '';
        const githubLink = item.querySelector('.github-link')?.href || '';
        
        markdown += `- **${fileName}** (${relevance} relevance)\n`;
        markdown += `  - Path: \`${filePath}\`\n`;
        if (githubLink) {
          markdown += `  - [View on GitHub](${githubLink})\n`;
        }
      });
      markdown += `\n`;
    }
  ...
  markdown += `\n*Generated by Repo-Trace AI-Powered Repository Intelligence*\n`;
  ...
  a.href = url;
  a.download = `repo-trace-chat-${timestamp}.md`;
